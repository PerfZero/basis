      ...
      <div className={styles.inner}>
        ...
        <div className={styles.grid}>
          {cards.map((card) => (
            <div
              key={card.id}
              className={card.isFeatured ? `${styles.card} ${styles.cardFeatured}` : styles.card}
            >
              {card.iconUrl && (
              ...
              {card.link && (
                <a href={card.link} className={styles.cardLink}>
                  {card.linkLabel} <svg width="10" height="8" viewBox="0 0 10 8" fill="none" xmlns="http://www.w3.org/2000/svg">
